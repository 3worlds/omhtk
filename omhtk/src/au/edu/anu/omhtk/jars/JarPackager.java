 ...
 *
 */
public abstract class JarPackager {
	...
	private Logger log = Logger.getLogger(JarPackager.class.getName());
	...
	public void addFile(String fileName, String jarDirectory) {
		...
		this.files.add(new JarFileRecord(fileName, jarDirectory + JarPackager.separator + file.getName()));
	}
...
//				System.out.println("ATTRIBUTES: "+manifest.getMainAttributes().getValue(Attributes.Name.CLASS_PATH));
			...
			for (String className : classNames) {
				...
				JarEntry jarEntry = new JarEntry(className.replaceAll("\\.", JarPackager.separator) + ".class");
...
//				String jarEntryString = jfr.jarName.replace("\\", Jar.separator);
				String jarEntryString = jfr.jarName.replaceAll("\\\\", JarPackager.separator);
				// we may need to check : mydir/file.txt as opposed to /mydir/file.txt
				if (jarEntryString.startsWith(JarPackager.separator)) {
					jarEntryString = jarEntryString.replaceFirst(JarPackager.separator, "");
				}
